...
$.fn.Validform = function () {
    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            if ($(this).hasClass('select')) {
                value = $(this).children('select').attr('relValue');
            }
            switch (checkexpession) {
                ...
                case "Decimal":
                    {
                        if (!isDeci(value)) {
                            Validatemsg = errormsg + "必须为整数或小数点后两位！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "Date":
                    {
                        if (!isDate(value, $(this).attr("eqvalue"))) {
                            Validatemsg = errormsg + "必须为日期格式 年-月-日！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
    ...
    //验证非正整数 nonPositiveInteger
    function isNonPositiveInteger(obj) {
        reg = /^((-\d+)|(0+))$/;
        if (!reg.test(obj)) {
            return false;
        } else {
    ...
    //验证负整数 negative integer
    function isNegativeInteger(obj) {
        reg = /^-[0-9]*[1-9][0-9]*$/;
        if (!reg.test(obj)) {
            return false;
        } else {
...
function ValidationMessage(obj, Validatemsg) {
    try {
        removeMessage(obj);
        obj.focus();
		layer.tips(Validatemsg, obj, { tips: 3});
        if (obj.hasClass('text') || obj.hasClass('select')) {
        ...
        if (obj.hasClass('select')) {
            $(document).click(function (e) {
                if (obj.children('select[type="select"]').attr('relValue')) {
                    removeMessage(obj);
                }
                e.stopPropagation();
            });
