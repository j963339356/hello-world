...
function displayPage(curPage) {
    ...
    CwHelper.Ajax("004300100048", body, function (result) {
        var data = result.body.items;
        $("#pageCount").val(Math.ceil(result.body.totalcount / 6));
        ...
        if (data == null || data.length==0) {
            ...
            $(data).each(function () {
                ulhtml += '<li>' + '<a href="CoachDetail.html?id=' + this.id + '&coach_num='+this.coach_num+'">' +
                    '<div class="tp"><img id="photofile" style="height: 140px;width: 130px;" src="../../../StaticResource/Theme2/Img/noimage.jpg"></div>' +
                    '<div class="ziliao">' +
                    '<p style="color:#28689a;">' + this.name + '</p>' +
                    '<p>驾校：' + this.corp_name + '</p>' +
                    '<p><span>准教车型：' + this.teach_permitted + '</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="margin-left:10%">性别：' + getSex(this.sex) + '</span></p>' +
                    '<p><span>教龄：</span>&nbsp;&nbsp;<span style="margin-left:10%">供职状态：</span></p>' +
                    '</div></a></li>';
            });

        }
        $("#coach_list").html(ulhtml);
